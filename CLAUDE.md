...
pnpm build        # Build for production (also runs pagefind indexing via postbuild)
pnpm preview      # Preview production build locally
pnpm check        # Run Astro type checking
pnpm format       # Format all files with Prettier
```

Note: `pnpm build` runs `astro build` followed by `pagefind --site dist` (search indexing). Search only works after a full build.
